...
#ndfd.setLocalCacheServerNDFD('http://ndfd.eas.cornell.edu/')
...
maxTime = datetime.utcnow() + timedelta(hours=250)
...
analysis = ndfd.getLocationData(var, lat, lon, elev=True, timeStep=timeStep, minTime=minTime, maxTime=maxTime, area='conus')

ndfd.plotData(analysis['variables'],'t2m')

'''
analysis = ndfd.getForecastAnalysis(var, lat, lon, elev=True, timeStep=timeStep, minTime=minTime, maxTime=maxTime, area='conus')
...
print('Lat: ' + str(analysis['gridLat']))
print('Lon: ' + str(analysis['gridLon']))
